...
def test_Google_says_drop_dead(mock_google, request):
    mock_google.return_value = False
    assert not _captcha_info_is_valid(request)


def test_Google_says_eva_thang_funky(mock_google, request):
    mock_google.return_value = True
    assert _captcha_info_is_valid(request)


def test_we_only_do_the_google_roundtrip_once(mock_google, request):
    mock_google.return_value = True
